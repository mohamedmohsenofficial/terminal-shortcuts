...
while ($CACHE_TARGETS.Count -gt 0) {
    ...
    Write-Host "  0. " -NoNewline; Write-Host "Skip" -ForegroundColor White -NoNewline; Write-Host " / " -NoNewline; Write-Host "Cancel" -ForegroundColor Red -NoNewline; Write-Host " Caches Cleanup`n"
    ...
    if ($choice -eq "1") {
        ...
        $excText = Read-Host "  Enter text to exclude"
        Write-Host ""
        $newTargets = @()
        ...
        Write-Host "  [SKIPPED]`n" -ForegroundColor White
        break 
    } else {
...
while ($ORPHANS.Count -gt 0) {
    ...
    Write-Host "  0. " -NoNewline; Write-Host "Skip" -ForegroundColor White -NoNewline; Write-Host " / " -NoNewline; Write-Host "Cancel" -ForegroundColor Red -NoNewline; Write-Host " Orphans Cleanup`n"
...
If (Get-Command docker -ErrorAction SilentlyContinue) {
    ...
    Write-Host "  [SKIPPED] Docker not found.`n" -ForegroundColor White 
}
